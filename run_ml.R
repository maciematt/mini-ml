...
run_caret <- function (X_y, learning_method, number_folds = 5, number_repeats = 10, parallelization = "local", sample_balance = "up", tune_length = 100, search = "random") {
...
main <- (function () {
  ...
  optimized_fit <- run_caret(X_y, number_folds = number_folds, number_repeats = number_repeats, sample_balance = sample_balance, learning_method = learning_method, parallelization = parallelization, tune_length = tune_length, search = search)
  ...
  # optimized_filtered <- filter_proba(optimized_fit)


  # lapply(1:length(optimized_filtered), function (x) {

  #   prediction_probs_gr <- optimized_filtered[[x]] %>% select(Zero, One)
  #   prediction_table_gr <- optimized_filtered[[x]] %>% select(obs, pred)
  #   prediction_table_gr$is_same <- prediction_table_gr$pred == prediction_table_gr$obs
  #   colnames(prediction_table_gr) <- c("ground_truth", "prediction", "is_same")
  #   prediction_table_gr$ground_truth <- ifelse(prediction_table_gr$ground_truth == "Zero", 0, 1)
  #   prediction_table_gr$prediction <- ifelse(prediction_table_gr$prediction == "Zero", 0, 1)

  #   f_name <- paste0(output_prefix, learning_method, "_", names(optimized_filtered)[x], ".pkl")

  #   save_to_pickle(list(prediction_table_gr, prediction_probs_gr, score_types, learning_type, learning_method, x), f_name)
  #   write(f_name, file = "./summary_file_names.txt", append = T)
